...
INITSH := $(LOCAL_PATH)/init.sh
	$(hide) cp $(TARGET_RECOVERY_ROOT_OUT)/etc/bootrec-device $(PRODUCT_OUT)/bootrec-device
BOOTREC_DEVICE := $(PRODUCT_OUT)/bootrec-device
...
$(INSTALLED_BOOTIMAGE_TARGET): $(PRODUCT_OUT)/kernel $(uncompressed_ramdisk) $(INSTALLED_RAMDISK_TARGET) $(INITSH) $(PRODUCT_OUT)/utilities/busybox $(PRODUCT_OUT)/utilities/extract_elf_ramdisk $(PRODUCT_OUT)/utilities/keycheck $(MKBOOTIMG) $(MINIGZIP) $(INTERNAL_BOOTIMAGE_FILES) $(INSTALLED_DTIMAGE_TARGET)
